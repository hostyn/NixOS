    ...
    home-manager.users.${vars.user}.programs = {
      zsh = {
        ...
        initExtra = "
          ...
          cdj() {
            if [ -z \"\${1}\" ]; then
              echo \"Usage: cdj <journal-name>\"
              return 1
            fi
            pushd ~/Documents/*/*/\${1}* > /dev/null
          }

          _cdj() {
            local -a suggestions
            str_suggestions=$(find ~/Documents -mindepth 3 -maxdepth 3 -type d -name '[0-9][0-9].[0-9][0-9]*' -printf '%f\n' | sort | sed 's/ /:/')
            suggestions=(\${(f)str_suggestions})

            _describe 'paths' suggestions
          }
          ...
          compdef _cdj cdj
          ${
          if config.custom.programs.yazi.enable
          then "
                  function y() {
